...
impl NodeKey {
    pub fn new(buffer: &[u8], offset: u64) -> Result<NodeKey,RegError> {
        let _offset = offset;
        if buffer.len() < 76 {
             return Err(RegError::validation_error("NodeKey buff is too small".into())) }
        let signature = LittleEndian::read_u16(&buffer[0..2]);
        let flags = NodeKeyFlags::from_bits_truncate(
            LittleEndian::read_u16(&buffer[2..4])
        );
         let last_written = match WindowsTimestamp(LittleEndian::read_u64(&buffer[12..20])).to_datetime()
        {
          Ok(time) => Some(time),
          Err(_) => None,
        };
    ...
    pub fn get_next_value<Rs: Read+Seek>(&mut self, reader: &mut Rs)->Result<Option<ValueKey>,RegError>{
        ...
        match self.value_key_list{
            ...
            _ => Err(RegError::validation_error("Should already have a value".into()))
        }
    }
            ...
            Some(ref mut cell_data) => {
                match **cell_data {
                    ...
                    ref _other => {
                        ...
                        return Err(RegError::validation_error("Unhandled sub key list".into()))
                    }
                }
            },
            None => {
                return Err(RegError::validation_error("Subkey List is None.".into()))
            }
        }
    }
    ...
    pub fn set_security_key<Rs: Read+Seek>(&mut self, reader: &mut Rs)->Result<(),RegError>{
        ...
        match cell.get_data()?{
            ...
            other => {
                return Err(RegError::validation_error( format!("Unexpected SK type: {:?}",other).into()));
            }
        }
    ...
    pub fn get_last_written(&self)-> &Option<DateTime::<Utc>>
    {
        &self.last_written
    }
}
...
mod tests {
    ...
    fn nodekey() {
        ...
        let cell = match Cell::new(&mut Cursor::new(&buffer),0){
            Ok(cell)=>cell,
            Err(error)=> return Err(RegError::validation_error(error.into()))
        };
