...
impl Cell {
    ...
    pub fn at_offset<Rs: Read+Seek>(reader: &mut Rs, offset: u64) -> Result<Cell,RegError> {
        ...
        //println!("size {}", (size.abs() -4) as usize); 
        if ((size.abs() -4) as usize) > 1024*1024*1024
        {
    ...
    pub fn get_signature(&self)->Result<CellSignature, RegError>
    {
        if self.data.len() < 2
        {
          return Err(RegError::validation_error("signature size is too short".into()))
        }
        let data = LittleEndian::read_u16(&self.data[0..2]);
        Ok(CellSignature::new(data))
    }
