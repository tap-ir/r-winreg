...
impl ValueKeyList{
    pub fn new(buffer: &[u8], value_count: u32, offset: u64) -> Result<ValueKeyList,RegError> {
        ...
        for i in 0..value_count {
            let o = (i*4) as usize;
            if o+4 > buffer.len(){
                return Err(RegError::validation_error(format!("error: value_count: {}; buffer: {:?}",value_count,buffer).into()))
            }
    ...
    pub fn get_next_value<Rs: Read+Seek>(&mut self, reader: &mut Rs)->Result<Option<ValueKey>,RegError>{
        if self.next_index >= self.value_offsets.len(){
            ...
            match Cell::at_offset(reader, cell_offset)?.get_data()?{
                ...
                other => Err(RegError::validation_error(format!("CellData is not type ValueKey: {:?}",other).into()))
            }
        }
    }
}
...
impl ValueKey {
    pub fn new(buffer: &[u8], offset: u64)->Result<ValueKey,RegError> {
        ...
        if 20 + value_name_size as usize > buffer.len()
        {
            return Err(RegError::validation_error("invalid value_name".into()))
        }
    ...
    pub fn read_value<Rs: Read+Seek>(&mut self, reader: &mut Rs)->Result<bool,RegError>{
        ...
        if !self.data_is_resident() {
            ...
            match cell.get_data()? {
                ...
                _other => {
                    return Err(RegError::validation_error("read_value_from_hive unhandled cell data type".into()))
    ...
    pub fn decode_data(&self)->Result<Option<Data>,RegError>{
        ...
        if data_len > 0 {
            if self.get_size() > data_len as u32 {
                ...
                return Err(RegError::validation_error("Size is greater than data".into()))
            }
